...
Editor profilů zobrazuje vlevo navigaci a vpravo hodnoty; u barev je třetí sloupec s přímým ANSI barevným náhledem. Vybereš položku a upravíš pouze ji; změny se zapisují až po volbě `Uložit a skončit`. Položka `← Zpět` vrací o úroveň výše. Při úpravě barvy se otevře paleta pojmenovaných barevných obdélníků a mezi barvami se listuje šipkami nahoru/dolů; Enter volbu potvrdí a text v obdélníku používá kontrastní barvu. Pokud je nainstalovaný [`gum`](https://github.com/charmbracelet/gum), použije se interaktivní výběr položek, jinak funguje textový fallback vhodný pro Termux. Heslo se zadává skrytě, prázdné heslo při úpravě existujícího profilu zachová původní hodnotu a token se nemění. Konfigurace se zapisuje atomicky s právy `600`.
